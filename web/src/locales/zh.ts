...
General：实体和关系提取提示来自 GitHub - microsoft/graphrag：基于图的模块化检索增强生成 (RAG) 系统`,
    ...
    flow: {
      ...
      dataProductDescription: `此组件用于从数据产品中检索相关信息。选择数据产品。如果没有检索到任何内容，将返回"空响应"。`,
      dataResourceDescription: `此组件用于从数据资源中检索相关信息。选择数据资源。如果没有检索到任何内容，将返回"空响应"。`,
      computingNodeDescription: `此组件用于从算力节点中检索相关信息。选择算力节点。如果没有检索到任何内容，将返回"空响应"。`,
