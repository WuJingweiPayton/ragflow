export default {
  translation: {
    ...
    login: {
      ...
      title: '开始构建您的数算协同应用',
      description:
        '中国数谷“三数一链+”数据空间平台数算协同子系统，基于从数据资源到数据产品的“乐高式”加工生产体系，利用模型库、插件库、工具库等资源库，构建“数据”到“AI”的多方协同价值共创流水线，实现与接入连接器、可信数据空间节点、智算中心等其他系统的无缝衔接，助力用户实现海洋、农业、消费等应用场景的数算协同应用。',
      review: '来自 500 多条评论',
    },
    header: {
      knowledgeBase: '知识库',
    ...
    knowledgeList: {
      welcome: '欢迎回来',
      description: '今天我们要使用哪个数据资源？',
      createKnowledgeBase: '创建数据资源',
      name: '名称',
      namePlaceholder: '请输入名称',
      doc: '文档',
...
General：实体和关系提取提示来自 GitHub - microsoft/graphrag：基于图的模块化检索增强生成 (RAG) 系统`,
    ...
    flow: {
      ...
      customDescription: '客制化组件描述',
      dataProductDescription: `此组件用于从可信数据空间的数据产品目录服务中调用数据产品供下游组件使用`,
      dataResourceDescription: `此组件用于从可信数据空间的数据资源目录服务中调用数据资源供下游组件使用`,
      computingNodeDescription: `此组件用于从智算中心各节点中选取符合算力配置和调度要求的算力服务供下游组件使用`,
...
以上就是你需要总结的内容。`,
      ...
      promptTip:
        '系统提示为大模型提供任务描述、规定回复方式，以及设置其他各种要求。系统提示通常与 key （变量）合用，通过变量设置大模型的输入数据。你可以通过斜杠或者 (x) 按钮显示可用的 key。',
      knowledgeBasesTip: '选择关联的数据资源，或者在下方选择包含数据资源ID的变量。',
      knowledgeBaseVars: '数据资源变量',
      code: '代码',
